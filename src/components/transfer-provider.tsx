...
export function TransferProvider({ children }: { children: ReactNode }) {
  ...
  const startSend = useCallback(async (file: File, receiverDeviceId: string) => {
    ...
    const transferItem: ActiveTransfer = {
      ...
      state: "negotiating",
      progress: { bytesTransferred: 0, totalBytes: file.size, percent: 0, speedBps: 0, speedAvgBps: 0, estimatedRemainingMs: null, chunkIndex: 0, totalChunks: 0, currentFileIndex: 0, fileCount: 1, currentFileName: file.name },
      sessionId,
    };
  ...
  const acceptTransfer = useCallback(async (sessionId: string) => {
    ...
    const transferItem: ActiveTransfer = {
      ...
      state: "negotiating",
      progress: { bytesTransferred: 0, totalBytes: 0, percent: 0, speedBps: 0, speedAvgBps: 0, estimatedRemainingMs: null, chunkIndex: 0, totalChunks: 0, currentFileIndex: 0, fileCount: 1, currentFileName: "" },
      sessionId,
    };
    setActiveTransfers((prev) => [...prev, transferItem]);
